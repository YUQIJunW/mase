...
class VerificationCase:
    ...
    norm_type=2
    # out_num = 8
    out_num = int((num - (kernal_size - 1) - 1) / stride + 1)

    def __init__(self, samples=2):
        self.m = nn.LPPool1d(norm_type=self.norm_type, kernel_size=self.kernal_size, stride=self.stride)
...
@cocotb.test()
async def cocotb_test_lppool1d(dut):
