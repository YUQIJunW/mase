...
class VerificationCase:
    ...
    out_h = 2
    out_num = out_h * out_w
    stride = int(in_w/out_w)
    kernal_w = int(in_w-stride*(out_w-1))
    kernal_h = int(in_h-stride*(out_h-1))

    def __init__(self, samples=2):
        self.m = nn.AvgPool2d(kernel_size=(self.kernal_w, self.kernal_h), stride=self.stride)
...
@cocotb.test()
async def cocotb_test_adaptiveavgpool2d(dut):
